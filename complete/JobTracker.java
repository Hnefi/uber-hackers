...
class PartitionThread implements Runnable 
{
    ...
    public PartitionThread(ZkConnector z, ZkPacket p,String s,String cpath,Integer i,JobTracker jt) {
        zkc = z;
        packet = p;
        activeJobPath = s;
        completedPath = cpath;
    ...
    public void run() {
        ...
        if ( incomingRequest instanceof ClientNewJobRequest ) {
            ...
            if (stat == null) { // then only create one partition
                ...
                Thread partThread = new Thread(new PartitionThread(zkc,toPartition,activeJobPath,completedJobPath,currentID,parentTracker),"PartitionThread1");
                partThread.start();
                ...
                for (int i = 1;i<=numPartitions;i++) {
                    ...
                    Thread iterThread = new Thread(new PartitionThread(zkc,toPartition,activeJobPath,completedJobPath,currentID,parentTracker),"PartitionThread"+i);
                    iterThread.start();
                }
